...
如果 $V$ 中的一个向量组既线性无关又张成 $V$，则称为 $V$ 的**基**。

$V$ 中的一个向量组 $v_1,...,v_n$ 是 $V$ 的基当且仅当 $V$ 中的每个向量 $v$ 都可以被唯一表示为 $v=a_1v_1+...+a_nv_n$（$a_i\in \mathbb{F}$）。（2.30）

证明：

> 

在向量空间中，每个张成组都可以化简为一个基。（2.31）
...
在有限维向量空间中，每个线性无关向量组都可以扩充为一组基。（2.33）

证明：

> 

对于有限维向量空间 $V$，如果 $U$ 是 $V$ 的子空间，那么存在 $V$ 的子空间 $W$ 使得 $V=U\oplus W$。（2.34）
...
有限维向量空间的任意基的长度相同。（2.35）

证明：

> 

有限维向量空间 $V$ 的**维度**定义为该向量空间的基的长度，记作 $dim\ V$。

如果 $V$ 是有限维向量空间，$U$ 是 $V$ 的子空间，那么 $dim\ U\le dim\ V$。（2.38）

证明：

> 

在有限维向量空间 $V$ 中，所有长度为 $dim\ V$ 的线性无关向量组都是 $V$ 的基。（2.39）

证明：

> 

在有限维向量空间 $V$ 中，所有长度为 $dim\ V$ 的张成组都是 $V$ 的基。（2.42）

证明：

> 

如果 $U_1$ 和 $U_2$ 是一个有限维向量空间的两个子空间，那么：$dim(U_1+U_2)=dim\ U_1+dim\ U_2-dim(U_1\cap U_2)$。（2.43）
